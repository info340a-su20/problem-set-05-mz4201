...
function createTaskItemElement(task){
  ...
  listElem.textContent = task.description;
...
function renderTaskList(){
  let orderListElem = document.querySelector('ol');
  orderListElem.innerHTML = "";
  for (let i of state.taskList){
    let list = createTaskItemElement(i);
    orderListElem.appendChild(list);
  }
  renderInput();
}
...
function addNewTask(){
  let id = 1;
  state.taskList.push({id:id ,description: state.inputtedText, complete: false});
  id = id + 1;
  state.inputtedText = "";
  renderTaskList();
}
...
function renderInput(){
  let inputElem = document.querySelector('input');
  inputElem.value = state.inputtedText;
